---
path: '/react-native-authentication-login-signup/'
...
seo:
  title: |
    Add Authentication to React Native with Open Source Examples
  description: |
    Add authentication (login, sign up, 2fa, jwt) to your react native application using open source ORY Kratos.
    Available as a React Native Expo template!
  keywords: |
...
This guide explains step-by-step how to add authentication (login, registration,
logout), user management (update profile, change password, forgot password), and
many other profile-related flows to a React Native application using the open
source [ORY Kratos](https://github.com/ory/kratos) Identity and User Management
Go server.
...
<p>
  <figure>
    ...
    <figcaption>
      React Native Mobile App with Authentication (Sign In, Sign Up) and User
      Management using ORY Kratos as a backend.
    </figcaption>
  </figure>
</p>
...
## React Native with Authentication (Login, Registration) and User Management for Expo
...
[ORY Kratos documentation](https://www.ory.sh/kratos/docs/cli/kratos). Because we are using
the playground in this example, we will use the [Ory CLI](https://github.com/ory/cli) instead as it allows us to connect
to Ory.

```shell session
$ docker run oryd/ory:v0.0.47 help
```
...
$ export ORY_ACCESS_TOKEN=RaEEZfMbx7QGuf0uWNt9c15NE4FG4OOW
$ docker run \
...
### React Native Sign In Component Example
...
### React Native Sign Up Component Example
...
## Writing Auth with Login and Registration for a React Native App From Scratch
