...
import {pb16, pb32, pb64, pb8, pt32} from "../components/freestanding/utils/padding.module.css";
...
export default function SummitTemplate(props: any) {
  ...
  const youtubeId = new URL(frontmatter.youtubelink).searchParams.get('v') || 'yaMRzF9arDc'
  ...
  return (
    <Layout isEvent={true} theme={'info'}>
      ...
      />
      <Container
        className={cn(styles.sessionList)}
        alignItems={'start'}
      >
        <h1 className={cn(pb64)}>{frontmatter.title}</h1>
        <YoutubeEmbed embedId={youtubeId}/>
        <p className={cn(pb32)}>{frontmatter.description}</p>
        <Button style={'filled'} to={frontmatter.presentationlink} openInNewWindow={true}>Download the
          Presentation</Button>
      </Container>
          ...
          <Grid lg={12} md={12} sm={12} xs={12} key={speaker.fullname}>
            <article className={(styles.speakerCardLarge)}>
              <Container className={styles.speakerCardDirection}>
                ...
                >
                  <article>
                    <Container key={speaker.social} className={cn(styles.speakerSocial, pb16)}>
                      <Container flexContainer={'column'} justify={'center'} alignItems={'center'}>
                        {speaker.speakerImage}
                        <Container flexContainer={'column'} alignItems={'center'}
                                   className={cn('text-is-centered', pt32)}>
                          <h6 className={cn(pb8)}>{speaker.fullname}</h6>
                          <p className={cn('font-p-small', styles.speakerName)}>{speaker.title} at {speaker.company}</p>
                        </Container>
                      </Container>
                    </Container>
                  </article>
