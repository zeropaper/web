...
Before reading on please note that this article is an **opinion piece**. My
intention is to save you valuable time developing the wrong things at the wrong
time. It is no way meant to discredit the technology and the brilliant people
working on them.
...
and problems that users encounter. As a company, we also help others audit and
vet their systems. Unfortunately, we often see these two protocols used in the
wrong context. This doesn't happen because people are "too dumb" or "don't
understand security". It happens because the protocols are complex and when used
in the wrong context - which is often the case - can lead to serious security
...
Let's look at the structure of this article.
...
1. We have utmost respect for the amount of innovation spawned by researchers
   ...
   really simple for the next five years!"
2. Ory is developing an open-source identity infrastructure service called
   ...
   of the reasons that initially pushed to develop this new system. So yes, it's
   a shameless self-plug. But also: it's what we believe! There is a place in
   the software world for a secure, reliable, scalable and **easy** solution.
   **Our goal at Ory is to develop an open standard, with the help of a
   brilliant community, that is better suited for the 80%**.

## Do You Need OAuth2 or OpenID Connect?

Before we answer that, let's agree that it's generally an excellent idea to use
an existing piece of software to solve login, registration, user management,
password, reset, account recovery, 2FA, and all topics related to user
authentication and permission management (authorization). Today most
technologies today rely on OAuth2 as the primary interaction protocol. And as we
will learn in this article, it is most often too complex with too many edge
cases.
...
OAuth 2.0 was invented to address a need in the era of Web 2.0. Platforms such
as Google and Facebook have massive troves of data and they wanted to open this
...
OpenID Connect (or they hide it really, really well) but instead a regular login
flow: Post the HTML form to a server, receive a cookie!
...
OAuth2 extensions. The list is long, not to make a point (well, sort of), but
...
active developed and draft status. Besides these more generic specs, there are
...
and does not apply to valid use cases. But before you jump into the cold water
you should know exactly how cold it is.
...
I started Ory open source a few years back and have since scaled it from a
one-man show to a company of almost 30 people (a comparatively small number in
comparison). Meanwhile, I've witnessed the community grow and struggle with
OAuth2 and OpenID Connect terminology, complexity, and tooling. In hindsight,
it's clear that scaling development (the human side) with complex protocols is
...
if you can build a team which is responsible for training, auditing, developing,
and maintaining your OAuth2 and OpenID Connect services and integration tools.
