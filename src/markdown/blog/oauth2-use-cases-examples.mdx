...
**You probably do not need OAuth2, nor OpenID Connect.** It's a controversial
...
While writing and maintaining these two projects I've spent a good portion of my
life working with a large auth\* community and understanding all the use cases
and problems that users encounter. As a company, we also help others audit and vet
their systems. Unfortunately, we often see patterns of misuse of these
two protocols. This doesn't happen because people are "too dumb" or "don't
understand security." It happens because the protocols are complex. When used
in the wrong context - which is very often the case - they can lead to
serious security vulnerabilities. So far we've identified over 12
cases where incorrect implementation of OAuth2 and OpenID Connect caused low-severity security issues (e.g. logout does not properly invalidate sessions) to
catastrophic vulnerabilities (account hijacking with full administrative
privilege escalation). The scary part is that it can happen to any company of any
size - regardless of how talented their developers are!

At Ory, we don't sell OAuth2 and OpenID Connect as a holy grail solution. While the
two are very powerful protocols when used correctly and have many advantages and
use cases, the truth is that they are not always necessary. In fact, you most likely do not need
them. Before we continue, be aware that there are two types of
...
OAuth2 and OpenID Connect receive new extensions and protocols seemingly
every month.

If you're here for a good time, not a long time, skip to the section you need
the most. But if you have the time, I encourage you to read everything!
...
1. We have utmost respect for the amount of innovation spawned by researchers and contributors in the area
   of OAuth2 and OpenID Connect and this article does not diminish their impressive work. We rather intend to shift the mindset of small and medium-sized teams from "we need
   this complex security thing" to "we can solve this complex thing at a later
   stage and use something really simple for the next five years!" 
2. Ory is developing an open-source identity infrastructure service called
   ...
   of the reasons that initially pushed to develop this new system. So yes, it's a
   shameless self-plug. But also: it's what we believe! There is a place
...
Before we answer that, let's agree that it's generally an excellent
...
most technologies today rely on OAuth2 as the primary interaction
protocol. But that doesn't mean that there isn't a better way to solve these
problems. And yes, incoming: a shameless plug! Ory's attempt at building a
...
OAuth 2.0 was invented to address a need in the era of Web 2.0. Platforms such as
Google and Facebook have massive troves of data and they wanted to open this
data up to third parties. This was sometimes used benevolently (e.g. Facebook Games)
...
2. Allow third parties to authenticate you via your e.g. Facebook account (OpenID
   Connect).
...
is implicit. In security, we really don't like implicit.
...
Tokens are transparent - usable - to the third party clients; it's because they are
...
interacts with third parties, such as partner networks, platform APIs? Or are you
...
solves things a little differently. GitHub does not offer OpenID Connect
...
to become compatible, and Auth0 has their own OAuth2 quirks... the list goes on and
on!

OAuth2 and OpenID Connect have spawned an enormous and exciting amount of innovation. That said, if you're a small team or have a very clear use
case (e.g. an app), the amount of reading needed to properly interface and use
the correct OAuth2 methodologies is vast - even if you are only interfacing
with OAuth2 servers and using OAuth2 libraries! Let's take a look at the
most popular OAuth2 extensions. The list is long, not to make a point (well, sort
of), but because there are so many variations and possibilities:
...
This list is long, but there is an even longer list of RFCs and specs
...
So the question is: do you really need all of this? Or should we simply solve login and move on until we actually need to address a critical
need?

## Unnecessary, Complex Use Cases

If we haven't convinced you yet, let's take a look at some more arguments for
avoiding OAuth2 and OpenID Connect. As stated earlier, this is a
recommendation and does not apply to valid use cases. But before you jump into the cold
water you should know exactly how cold it is. 
...
I started Ory open source a few years back and have since scaled it from a one-man show to a company of almost 30 people
(a comparatively small number in comparison). Meanwhile, I've witnessed the community grow and
struggle with OAuth2 and OpenID Connect terminology, complexity, and tooling. In hindsight, it's clear that scaling development (the human side) with complex protocols is
...
A good question to ask yourself when deciding whether or not you need OAuth2 / OpenID
Conenct is this:
...
those never made it past the draft status and therefore I didn't include them.
...
In conclusion: OAuth2 issues tokens. But not every token must be issued by OAuth2
to be secure.
...
if you can build a team which is responsible for training, auditing,
...
And if you still decide to go this route, be prepared to spend a
considerable amount of time and development resources to get it right.
...
for Ory to really become a new standard, we already have a solid
foundation! While you're here, maybe one of our projects will spark your
interest:
...
There are many examples of these two protocols being used securely
...
you learned a thing or two and, more than anything, that you now have the knowledge
to start building your system without spending restless nights on things
you don't need yet, figuring out all the intricate details of OAuth2 and
OpenID Connect!

Thanks for reading and see you next time, maybe in the [Ory Community](https://slack.ory.sh)!
